...
function getOptions(opts) {
  ...
  for (var opt in defaultOptions)
    { options[opt] = opts && has$3(opts, opt) ? opts[opt] : defaultOptions[opt]; }
...
pp$2.checkLValSimple = function(expr, bindingType, checkClashes) {
  ...
  case "Identifier":
    ...
    if (isBind) {
      ...
      if (checkClashes) {
        if (has$3(checkClashes, expr.name))
...
pp$8.regexp_validateUnicodePropertyNameAndValue = function(state, name, value) {
  if (!has$3(state.unicodeProperties.nonBinary, name))
    { state.raise("Invalid property name"); }
...
async function parse(filePath) {
  try {
    ...
    const parse = Parser.parse(contents, DEFAULT_ACORN_OPTIONS);
...
var iterateIterator = function iterateIterator(iterator) {
	if (!iterator || typeof iterator.next !== 'function') {
		throw new $TypeError$f('iterator must be an object with a `next` method');
	}
	if (arguments.length > 1) {
		var callback = arguments[1];
		if (typeof callback !== 'function') {
			throw new $TypeError$f('`callback`, if provided, must be a function');
		}
	}
	var values = callback || [];
	var result;
...
var iterateValue = function iterateValue(iterable) {
	var iterator = node(iterable);
	if (!iterator) {
		throw new $TypeError$e('non-iterable value provided');
	}
	if (arguments.length > 1) {
...
function isArray(obj) { return toStr$5(obj) === '[object Array]'; }
function isDate(obj) { return toStr$5(obj) === '[object Date]'; }
function isRegExp(obj) { return toStr$5(obj) === '[object RegExp]'; }
function isError(obj) { return toStr$5(obj) === '[object Error]'; }
function isSymbol$1(obj) { return toStr$5(obj) === '[object Symbol]'; }
function isString$1(obj) { return toStr$5(obj) === '[object String]'; }
function isNumber(obj) { return toStr$5(obj) === '[object Number]'; }
function isBigInt(obj) { return toStr$5(obj) === '[object BigInt]'; }
function isBoolean(obj) { return toStr$5(obj) === '[object Boolean]'; }
...
var isCallable$1 = reflectApply
	...
	: function isCallable(value) {
		...
		if (typeof value !== 'function' && typeof value !== 'object') { return false; }
		if (typeof value === 'function' && !value.prototype) { return true; }
		if (hasToStringTag$4) { return tryFunctionObject$1(value); }
		if (isES6ClassFn$1(value)) { return false; }
		var strClass = toStr$3.call(value);
		return strClass === fnClass$1 || strClass === genClass$1;
	};
...
var ToPropertyDescriptor = function ToPropertyDescriptor(Obj) {
	if (Type(Obj) !== 'Object') {
		throw new $TypeError$9('ToPropertyDescriptor requires an object');
	}
	...
	if (src(Obj, 'get')) {
		...
		if (typeof getter !== 'undefined' && !IsCallable(getter)) {
			throw new $TypeError$9('getter must be a function');
		}
		desc['[[Get]]'] = getter;
	}
	if (src(Obj, 'set')) {
		...
		if (typeof setter !== 'undefined' && !IsCallable(setter)) {
			throw new $TypeError$9('setter must be a function');
		}
		desc['[[Set]]'] = setter;
	}

	if ((src(desc, '[[Get]]') || src(desc, '[[Set]]')) && (src(desc, '[[Value]]') || src(desc, '[[Writable]]'))) {
		throw new $TypeError$9('Invalid property descriptor. Cannot both specify accessors and a value or writable attribute');
	}
	return desc;
};
...
var hasToStringTag$3 = hasSymbols$1 && typeof Symbol.toStringTag === 'symbol';
var has;
var $exec;
var isRegexMarker;
var badStringifier;
...
var implementation$1 = function map(callbackfn) {
	var O = ToObject(this);
...
var polyfill$2 = function getPolyfill() {
	...
	return esArrayMethodBoxesProperly(method) ? method : implementation$1;
};
...
var implementation = function allSettled(iterable) {
	var C = this;
	if (Type(C) !== 'Object') {
...
var shim = function shimAllSettled() {
	requirePromise();

	var polyfill$1 = polyfill();
	defineProperties_1(Promise, { allSettled: polyfill$1 }, {
		allSettled: function testAllSettled() {
			return Promise.allSettled !== polyfill$1;
		}
	});
	return polyfill$1;
};
 ...
 */
async function pathExists(paths) {
  try {
    await promise_allsettled(paths.map((path) => fs.promises.access(path)));
  } catch (e) {
  }
...
function repath(originalPath, newExtension) {
  return path__namespace.join(path__namespace.dirname(originalPath), path__namespace.basename(originalPath, path__namespace.extname(originalPath)) + newExtension);
}
  ...
  await asyncWalk(program, {
    enter: async function (node) {
      if (node.type === 'ImportDeclaration' || node.type === 'ExportNamedDeclaration' || node.type === 'ExportAllDeclaration') {
        ...
        const basePath = path__namespace.join(dirname, source.value);
 ...
 */
async function format(configFileLocation, config) {
  const basePath = path__namespace.resolve(path__namespace.dirname(configFileLocation), config.compilerOptions.outDir);
  const filePaths = await glob__default['default'](basePath + '/**/*.js');
  const newFilePaths = new Set();
  for (const filePath of filePaths) {
    try {
      const newFilePath = repath(filePath, '.mjs');
      const fileDirName = path__namespace.dirname(filePath);
...
function config(project) {
  const configFilePath = project ? path.resolve(process.cwd(), project) : typescript__default['default'].findConfigFile(process.cwd(), typescript__default['default'].sys.fileExists);
  const configFile = typescript__default['default'].readConfigFile(configFilePath, typescript__default['default'].sys.readFile);
